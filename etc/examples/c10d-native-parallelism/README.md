...
This is a 2 convolutional layers model using native pytorch method **dist.init_process_group** and **DistributedDataParallel** to sync and join all the worker nodes.
...
Create the code that will train your model. We will use `train_dist_parallel.py` in the folder "c10d-native-parallelism" as an example
...
```bash
cd <path to FfDL repo>/etc/example/c10d-native-parallelism
