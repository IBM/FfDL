...
func (o *GetEventEndpointURL) Build() (*url.URL, error) {
	...
	if endpointID != "" {
		...
		return nil, errors.New("EndpointID is required on GetEventEndpointURL")
	}
	eventType := o.EventType
	if eventType != "" {
		_path = strings.Replace(_path, "{event_type}", eventType, -1)
	} else {
		return nil, errors.New("EventType is required on GetEventEndpointURL")
	}
	modelID := o.ModelID
	if modelID != "" {
		_path = strings.Replace(_path, "{model_id}", modelID, -1)
	} else {
		return nil, errors.New("ModelID is required on GetEventEndpointURL")
	}
...
func (o *GetEventEndpointURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetEventEndpointURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetEventEndpointURL")
	}
