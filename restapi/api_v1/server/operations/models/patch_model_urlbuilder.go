...
func (o *PatchModelURL) Build() (*url.URL, error) {
	...
	var _path = "/v1/models/{model_id}"

	modelID := o.ModelID
	if modelID != "" {
		_path = strings.Replace(_path, "{model_id}", modelID, -1)
	} else {
		return nil, errors.New("ModelID is required on PatchModelURL")
	}
...
func (o *PatchModelURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PatchModelURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PatchModelURL")
	}
